...
export default function CandidateProfileView({
  ...
  return (
    <motion.div 
      ...
      className={`fixed inset-0 z-50 overflow-y-auto min-h-screen px-4 py-8 sm:px-6 lg:px-12 backdrop-blur-3xl transition-colors duration-400 ${
    ...
    >
      <div className="max-w-7xl mx-auto space-y-6 pb-24">
        ...
        {/* Hero Section Banner */}
        <div className={`relative border rounded-2xl overflow-hidden p-6 sm:p-8 transition-all duration-300 shadow-xl ${
          isDarkMode ? 'bg-gradient-to-br from-[#0c1322] via-[#090e1a] to-[#040811] border-white/15' : 'bg-white border-[#AAB9C2]/45'
        }`}>
          ...
          <div className="absolute top-0 right-0 p-8 opacity-5 font-mono select-none pointer-events-none hidden lg:block">
            <BdpLogo className="h-56 w-auto" isDarkMode={isDarkMode} />
          </div>
          ...
          <div className="relative flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div className="flex flex-col sm:flex-row items-center gap-5 sm:gap-6 text-center sm:text-left w-full md:w-auto">
              {applicant.avatarUrl ? (
                <img 
                  ...
                  alt={applicant.name} 
                  className={`w-28 h-28 rounded-2xl object-cover border-2 shadow-xl ${
                    isDarkMode ? 'border-cyan-500/40' : 'border-[#004a8f]/30'
                  }`}
                  referrerPolicy="no-referrer"
                />
              ) : (
                <div className={`w-28 h-28 rounded-2xl bg-gradient-to-tr from-[#004a8f] to-[#00b0d8] flex items-center justify-center font-bold text-white text-4xl shadow-xl`}>
              ...
              <div className="space-y-1.5">
                <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold font-mono border ${getStatusStyle(pipelineStatus)}`}>
                    {getStatusLabelText(pipelineStatus)}
                  </span>
                </div>
                
                <p className={`text-sm font-semibold tracking-wide flex items-center justify-center sm:justify-start gap-1.5 ${
                  isDarkMode ? 'text-cyan-400' : 'text-[#005baa]'
                }`}>
                  <Briefcase className="w-4 h-4 text-[#00b0d8]" />
                  <span>{applicant.role}</span>
                </p>

                <div className="flex flex-wrap items-center justify-center sm:justify-start gap-x-4 gap-y-1.5 text-xs text-slate-500">
                  ...
                  <span className="flex items-center gap-1">
                    <GraduationCap className="w-3.5 h-3.5" /> {applicant.education}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className={`w-full md:w-auto p-4 rounded-xl border flex flex-col md:items-end justify-between gap-3 ${
              isDarkMode ? 'bg-[#0f172a] border-white/5' : 'bg-slate-50 border-slate-200'
            }`}>
              <div className="flex items-center gap-1.5 justify-between md:justify-end font-mono text-[11px] text-slate-400">
                <span>Calificación General Simulada:</span>
                <span className={`text-lg font-bold ${isDarkMode ? 'text-cyan-400' : 'text-[#004a8f]'}`}>
                  {fitScoreCalculated}%
                </span>
              </div>
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`flex-1 md:flex-none px-4 py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition-all shadow-md cursor-pointer ${
                    isDarkMode 
                      ? 'bg-gradient-to-r from-[#004a8f] to-[#00b0d8] hover:from-[#005baa] hover:to-[#00b0d8] text-white border border-white/10 shadow-[0_4px_15px_rgba(0,176,216,0.25)]' 
                      : 'bg-[#004a8f] hover:bg-[#003d75] text-white border border-transparent'
                  }`}
                >
